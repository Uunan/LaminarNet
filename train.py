...
def run_test():
    ...
    if os.path.exists(trans_path):
        ...
        print("   ⚠️ Ağırlıklar bulunamadı, hız testi için rastgele ağırlıklar kullanılacak.")
    transformer.eval()
    t_params = sum(p.numel() for p in transformer.parameters()) / 1e6
    print(f"   ℹ️ Boyut: {t_params:.1f}M parametre")
    ...
    print(f"   ℹ️ Boyut: {l_params:.1f}M parametre")
